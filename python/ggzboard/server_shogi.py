...
import bogaprot
from module_shogi import *

class ShogiServer(bogaprot.BogaprotServer):
	def __init__(self):
	...
	def hook_join(self, num, type, name, fd):
		print "(shogi) * join:", num, type, name, fd
		self.send_seat(num)
	...
	def hook_data(self, num, type, name, fd):
		print "(shogi) * data:", num, type, name, fd
		self.init(fd)
		op = self.getbyte()
		print "opcode", op
		if op == self.MSG_MOVE:
