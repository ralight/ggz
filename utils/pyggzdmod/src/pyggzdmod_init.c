...
static void pyggzdmod_cb_player_hook(GGZdMod *ggzdmod, GGZdModEvent event, void *handler_data);
...
static PyObject *pyggzdmod_cb_player = NULL;
...
static PyObject *pyggzdmod_set_handler(PyObject *self, PyObject *args)
{
	...
	switch(id)
	{
		...
		case GGZDMOD_EVENT_PLAYER_DATA:
			Py_XDECREF(pyggzdmod_cb_player);
			pyggzdmod_cb_player = temp;
			ggzdmod_set_handler(ggzdmod, GGZDMOD_EVENT_PLAYER_DATA, pyggzdmod_cb_player_hook);
			break;
...
void pyggzdmod_cb_player_hook(GGZdMod *ggzdmod, GGZdModEvent event, void *handler_data)
{
	PyObject *arg, *res;

	arg = Py_BuildValue("(i)", *(int*)handler_data);
	res = PyEval_CallObject(pyggzdmod_cb_player, arg);
	if(res == NULL)
	{
		printf("----------------------------\n");
		printf("ERROR in pyggzdmod callback!\n");
...
void initggzdmod(void)
{
	...
	PyModule_AddIntConstant(mod, "EVENT_PLAYER", GGZDMOD_EVENT_PLAYER_DATA);
