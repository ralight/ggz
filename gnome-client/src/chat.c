	...
	if(ignore == FALSE)
	{
		...
		switch(id)
		{
			case CHAT_MSG:
				if(!strncasecmp(message, "/me ", 4))
				{
					name = g_strdup_printf("%s %s", player, message+4);
				        gtk_xtext_append_indent(GTK_XTEXT(tmp)->buffer, "*", 1, name, strlen(name));
				} else {
					name = g_strdup_printf("<\003%s%s\003>", chat_get_color(player, message),  player);
				        gtk_xtext_append_indent(GTK_XTEXT(tmp)->buffer, name, strlen(name), message, strlen(message));
				}
			...
			case CHAT_LOCAL_NORMAL:
			        gtk_xtext_append_indent(GTK_XTEXT(tmp)->buffer, "---", 3, message, strlen(message));
				break;
...
void chat_send_prvmsg(GGZServer *server, gchar *message)
{
	...
	if (!success) {
		chat_display_message(CHAT_LOCAL_NORMAL, NULL, "Usage: /msg <username> <message>");
		chat_display_message(CHAT_LOCAL_NORMAL, NULL, "    Sends a private message to a user on the network.");
	}
...
void chat_enter(gchar *player)
{
        ...
        {
	        tmp = lookup_widget(interface, "custChatXText");
        	gtk_xtext_append_indent(GTK_XTEXT(tmp)->buffer, "-->", 3, player, strlen(player));
	}
}
