...
void clock_option_select(GtkButton * button, gpointer user_data)
{
	...
	mytime += gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(time_w));

	if (clocktype == CHESS_CLOCK_NOCLOCK)
		net_send_time(0);
	else
		net_send_time((clocktype << 24) + mytime);
