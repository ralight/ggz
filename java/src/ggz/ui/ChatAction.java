...
public abstract class ChatAction extends AbstractAction {
	...
	private void admin_kick(int commandIndex, String cmd) throws IOException {
		try {
			...
			if (sendAdmin(AdminType.GGZ_ADMIN_KICK, nickname, reason)) {
				chatPanel.appendCommandText(MessageFormat.format(messages
						.getString("ChatAction.Result.kick"),
						new Object[] { nickname }));
			}
		} catch (IndexOutOfBoundsException ex) {
			/* Could not parse it. */
	...
	private void admin_gag(int commandIndex, String cmd) throws IOException {
		try {
			...
			if (sendAdmin(AdminType.GGZ_ADMIN_GAG, nickname, null)) {
				chatPanel.appendCommandText(MessageFormat.format(messages
						.getString("ChatAction.Result.gag"),
						new Object[] { nickname }));
			}
		} catch (IndexOutOfBoundsException ex) {
			/* Could not parse it. */
	...
	private void admin_ungag(int commandIndex, String cmd) throws IOException {
		try {
			...
			// Assume that spaces are not allowed in names.
			String nickname = cmd.substring(0);
			if (sendAdmin(AdminType.GGZ_ADMIN_UNGAG, nickname, null)) {
				chatPanel.appendCommandText(MessageFormat.format(messages
						.getString("ChatAction.Result.ungag"),
						new Object[] { nickname }));
			}
		} catch (IndexOutOfBoundsException ex) {
			/* Could not parse it. */
	...
	private void admin_ban(int commandIndex, String cmd) throws IOException {
		try {
			...
			// Assume that spaces are not allowed in names.
			String nickname = cmd.substring(0);
			if (sendAdmin(AdminType.GGZ_ADMIN_BAN, nickname, null)) {
				chatPanel.appendCommandText(MessageFormat.format(messages
						.getString("ChatAction.Result.ban"),
						new Object[] { nickname }));
			}
		} catch (IndexOutOfBoundsException ex) {
			/* Could not parse it. */
