...
public class BrowserLauncher {
     ...
     */
    private static final String FINDER_TYPE = "FNDR"; //$NON-NLS-1$

    /**
     * The creator code of the Finder on a Macintosh, which is needed to send
     * AppleEvents to the application.
     */
    private static final String FINDER_CREATOR = "MACS"; //$NON-NLS-1$

    /** The name for the AppleEvent type corresponding to a GetURL event. */
    private static final String GURL_EVENT = "GURL"; //$NON-NLS-1$

    /**
     * The first parameter that needs to be passed into Runtime.exec() to open
     * the default web browser on Windows.
     */
    private static final String FIRST_WINDOWS_PARAMETER = "/c"; //$NON-NLS-1$

    /** The second parameter for Runtime.exec() on Windows. */
    private static final String SECOND_WINDOWS_PARAMETER = "start"; //$NON-NLS-1$
     ...
     */
    private static final String THIRD_WINDOWS_PARAMETER = "\"\""; //$NON-NLS-1$

    /**
     * The shell parameters for Netscape that opens a given URL in an
     * already-open copy of Netscape on many command-line systems.
     */
    private static final String NETSCAPE_REMOTE_PARAMETER = "-remote"; //$NON-NLS-1$

    private static final String NETSCAPE_OPEN_PARAMETER_START = "'openURL("; //$NON-NLS-1$

    private static final String NETSCAPE_OPEN_PARAMETER_END = ")'"; //$NON-NLS-1$
    ...
    static {
        ...
        if (osName.startsWith("Mac OS")) { //$NON-NLS-1$
            String mrjVersion = System.getProperty("mrj.version"); //$NON-NLS-1$
            ...
            try {
                ...
                if (version == 2) {
                    ...
                    errorMessage = "Unsupported MRJ version: " + version; //$NON-NLS-1$
                }
            } catch (NumberFormatException nfe) {
                loadedWithoutErrors = false;
                errorMessage = "Invalid MRJ version: " + mrjVersion; //$NON-NLS-1$
            }
        } else if (osName.startsWith("Windows")) { //$NON-NLS-1$
            if (osName.indexOf("9") != -1) { //$NON-NLS-1$
                jvm = WINDOWS_9x;
            } else {
                jvm = WINDOWS_NT;
            }
    ...
    private static Object locateBrowser() {
        ...
        case OTHER:
        default:
            String[] browsers = new String[] { "x-www-browser", "firebird",
                    "mozilla", "opera", "galeon", "konqueror", "netscape" };
    ...
    public static void openURL(String url) throws IOException {
        ...
        case WINDOWS_9x:
            ...
            break;
        case OTHER:
            // This is apparently the KDE command
            // `kfmclient openURL $uri` or `kmailservice $uri`
