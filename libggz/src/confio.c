 ...
 */
char * ggz_confio_read_string(int handle, const char *section,
				   const char *key, const char *def)
{
	_ggz_confio_file	*f_data;
	GGZListEntry	*s_entry, *e_entry;
	_ggz_confio_section	*s_data;
	_ggz_confio_entry	e_srch, *e_data;
 ...
 */
int ggz_confio_read_int(int handle, const char *section, const char *key, int def)
{
	char	tmp[20], *tmp2;
	int	value;

	sprintf(tmp, "%d", def);
	tmp2 = ggz_confio_read_string(handle, section, key, tmp);
	value = atoi(tmp2);
	free(tmp2);
 ...
 *	  - -1 on error
 */
int ggz_confio_read_list(int handle, const char *section, const char *key,
			      int *argcp, char ***argvp)
{
	...
	str = ggz_confio_read_string(handle, section, key, NULL);
		...
		*argvp = (char **) calloc((*argcp + 1), sizeof(char *));
		...
		do {
			...
			if((tmp2 = malloc(p-tmp+1)) == NULL)
				_ggz_error_sys_exit("malloc failed in ggz_confio_read_list");
 ...
 *	  - -1 on error
 */
int ggz_confio_write_string(int handle, const char *section,
				 const char *key, const char *value)
{
	_ggz_confio_file	*f_data;
	GGZListEntry	        *s_entry;
	_ggz_confio_section	*s_data;
	_ggz_confio_entry	e_data;
	...
	if(!f_data->writeable) {
		_ggz_debug(GGZ_DBG_CONF,
		      "ggz_confio_write_string: file is read-only");
		return -1;
	}
	...
	if(s_entry == NULL) {
		...
		if(ggz_list_insert(f_data->section_list, (void*)section) < 0) {
			_ggz_debug(GGZ_DBG_CONF,
			      "ggz_confio_write_string: insertion error");
			return -1;
		}
		s_entry = ggz_list_search(f_data->section_list, (void*)section);
	}
 ...
 *	  - -1 on failure
 */
int ggz_confio_write_int(int handle, const char *section, const char *key, int value)
{
	char	tmp[20];

	sprintf(tmp, "%d", value);
	return ggz_confio_write_string(handle, section, key, tmp);
}


/* ggz_confio_write_list()
 ...
 *	  - -1 on failure
 */
int ggz_confio_write_list(int handle, const char *section, 
			       const char *key, int argc, char **argv)
{
	...
	*dst = '\0';
	...
	return ggz_confio_write_string(handle, section, key, buf);
}


/* ggz_confio_remove_section()
 ...
 *	  - -1 on failure
 */
int ggz_confio_remove_section(int handle, const char *section)
{
	_ggz_confio_file	*f_data;
	GGZListEntry	*s_entry;
	...
	if(!f_data->writeable) {
		_ggz_debug(GGZ_DBG_CONF,
		      "ggz_confio_remove_section: file is read-only");
		return -1;
	}
 ...
 *	  - -1 on failure
 */
int ggz_confio_remove_key(int handle, const char *section, const char *key)
{
	_ggz_confio_file	*f_data;
	GGZListEntry	*s_entry, *e_entry;
	_ggz_confio_section	*s_data;
	...
	if(!f_data->writeable) {
		_ggz_debug(GGZ_DBG_CONF,
		      "ggz_confio_remove_key: file is read-only");
		return -1;
	}
 ...
 *	  - -1 on failure
 */
int ggz_confio_commit(int handle)
{
	_ggz_confio_file	*f_data;
	GGZListEntry	*s_entry, *e_entry;
	_ggz_confio_section	*s_data;
	...
	if(!f_data->writeable) {
		_ggz_debug(GGZ_DBG_CONF,
		      "ggz_confio_commit: file is read-only");
		return -1;
	}
...
void ggz_confio_close(int handle)
{
	GGZListEntry	*f_entry;
	_ggz_confio_file	*f_data=NULL;
...
int _ggz_confio_parse(const char *path, const unsigned char options)
{
	...
	_ggz_confio_file	*file_data=NULL;
	...
	if(opt_rdonly && (opt_rdwr || opt_create)) {
		_ggz_debug(GGZ_DBG_CONF,
			      "ggz_confio_parse: Invalid options");
		return -1;
	}
	...
	file_data = ggz_malloc(sizeof(_ggz_confio_file));
...
static GGZList * file_parser(const char *path)
{
	...
	GGZList		*s_list;
	GGZListEntry	*s_entry;
	_ggz_confio_section	*s_data=NULL;
	_ggz_confio_entry	*e_data=NULL;
	...
	e_data = ggz_malloc(sizeof(_ggz_confio_entry));
...
static char *dup_string(const char *src)
{
	...
	if((dst = malloc(strlen(src) + 1)) == NULL)
		_ggz_error_sys_exit("malloc failed in dup_string()");
...
static void *section_create(void *data)
{
	...
	/* is not expected to be the full struct, but just the name str  */
	_ggz_confio_section	*dst;

	dst = ggz_malloc(sizeof(_ggz_confio_section));
...
static void *entry_create(void *data)
{
	_ggz_confio_entry	*src, *dst;

	src = data;
	dst = ggz_malloc(sizeof(_ggz_confio_entry));
