...
static char file_unit_name[12][36] = {"flag", "bomb", "spy", "scout", "miner", "sergeant", "lieutenant", "captain", "major", "colonel", "general", "marshall"};
...
int map_save(combat_game *map) {
  ...
  if (handle < 0) {
    ...
    if (handle < 0) {
      ...
      sprintf(filename, "%s/.ggz/combat", getenv("HOME"));
      mkdir(filename, S_IRWXU | S_IRGRP | S_IXGRP );
...
void map_load(combat_game *_game, char *filename, int *changed) {
  ...
  for (a = 0; a < 12; a++) {
    ARMY(_game, a) = ggz_conf_read_int(handle, "army", file_unit_name[a], 0);
  }
  // Terrain data
