...
GtkWidget* create_dlg_yesno(const char *question)
{
  ...
  gtk_object_set_data (GTK_OBJECT (dlg_yesno), "dialog_vbox1", dialog_vbox1);
  gtk_widget_show (dialog_vbox1);

  vbox1 = gtk_vbox_new (FALSE, 0);
  gtk_widget_ref (vbox1);
  gtk_object_set_data_full (GTK_OBJECT (dlg_yesno), "vbox1", vbox1,
  ...
  gtk_object_set_data_full (GTK_OBJECT (dlg_yesno), "label3", label3,
                            (GtkDestroyNotify) gtk_widget_unref);
  gtk_widget_show (label3);
  gtk_box_pack_start (GTK_BOX (vbox1), label3, FALSE, FALSE, 0);

  dialog_action_area1 = GTK_DIALOG (dlg_yesno)->action_area;
  gtk_object_set_data (GTK_OBJECT (dlg_yesno), "dialog_action_area1", dialog_action_area1);
  gtk_widget_show (dialog_action_area1);
  gtk_container_set_border_width (GTK_CONTAINER (dialog_action_area1), 10);

  yes_button = gtk_button_new_with_label (_("Yes"));
  gtk_widget_ref (yes_button);
  gtk_object_set_data_full (GTK_OBJECT (dlg_yesno), "yes_button", yes_button,
                            (GtkDestroyNotify) gtk_widget_unref);
  gtk_widget_show (yes_button);
  gtk_box_pack_start (GTK_BOX (dialog_action_area1), yes_button, FALSE, FALSE, 0);
  gtk_widget_set_usize (yes_button, 64, -2);

  no_button = gtk_button_new_with_label (_("No"));
  gtk_widget_ref (no_button);
  gtk_object_set_data_full (GTK_OBJECT (dlg_yesno), "no_button", no_button,
                            (GtkDestroyNotify) gtk_widget_unref);
  gtk_widget_show (no_button);
  gtk_box_pack_start (GTK_BOX (dialog_action_area1), no_button, FALSE, FALSE, 0);
  gtk_widget_set_usize (no_button, 64, -2);

  gtk_signal_connect_object (GTK_OBJECT (dlg_yesno), "delete_event",
                             GTK_SIGNAL_FUNC (gtk_widget_destroy),
                             GTK_OBJECT (dlg_yesno));
  gtk_signal_connect_object (GTK_OBJECT (yes_button), "clicked",
                             GTK_SIGNAL_FUNC (yesno_yes),
                             NULL);
