 ...
 *
 * JDS: Well, the README is gone (though it can be found in the
 * NetSpades 4.2 sources).  This file _is_ necessary with GTK1.2,
 * since it provides the "input" and "output" signals which are used to
 * make a "bid" of -1 appear as "nil" (see SpinInput() and SpinOutput()
 * in gtk_play.c).  GTK2.0 has these signals, but they don't seem to work
 * the same way.  Regardless, this file cannot be compiled with GTK2.
 */
...
#ifndef GTK2 /* Must be below "#include <config.h>" */
