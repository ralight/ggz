 ...
 * $Id: main_win.c 6270 2004-11-05 19:26:41Z jdorje $
 *
...
static GtkWidget *create_menus(GtkWidget *window)
{
	...
	gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menu_item),
				       TRUE);
	g_object_set_data(G_OBJECT(window), "auto_call_flag", menu_item);
...
GtkWidget *create_main_win(void)
{
  ...
  g_object_set_data_full(G_OBJECT (main_win), "white_arrow", white_arrow,
			 (GtkDestroyNotify) gtk_widget_unref);
  ...
  g_object_set_data_full(G_OBJECT (main_win), "white_time", white_time,
			 (GtkDestroyNotify) gtk_widget_unref);
  ...
  g_object_set_data_full(G_OBJECT (main_win), "black_arrow", black_arrow,
			 (GtkDestroyNotify) gtk_widget_unref);
  ...
  g_object_set_data_full(G_OBJECT (main_win), "black_time", black_time,
			 (GtkDestroyNotify) gtk_widget_unref);
  ...
  g_object_set_data_full(G_OBJECT (main_win), "last_moves", last_moves,
			 (GtkDestroyNotify) gtk_widget_unref);
  ...
  g_signal_connect(board, "expose_event",
		   GTK_SIGNAL_FUNC (on_board_expose_event), NULL);
  g_signal_connect(board, "button_press_event",
		   GTK_SIGNAL_FUNC (on_board_button_press_event), NULL);
  g_signal_connect(board, "drag_begin",
		   GTK_SIGNAL_FUNC (on_board_drag_begin), NULL);
  g_signal_connect(board, "drag_motion",
		   GTK_SIGNAL_FUNC (on_board_drag_motion), NULL);
