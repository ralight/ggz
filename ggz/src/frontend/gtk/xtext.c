	...
	while (*text)
	{
		...
		} else
		{
			...
			case ATTR_UNDERLINE:
				break;
			default:
				xx += XTextWidth (xtext->xfont, text, 1);
				if (xx >= x)
							...
							int *out_of_bounds)
{
	...
	*off =
		gtk_xtext_find_x (xtext, x, ent, subline, line, win_width,
								out_of_bounds);
...
gtk_xtext_selection_draw (GtkXText * xtext, int draw)
{
	...
	if (draw)
	{
		...
		} else
		{
			/*if (xtext->io_tag == -1)
			{*/
				gtk_xtext_entrender_timeout (xtext);
				xtext->old_ent_start = xtext->last_ent_start;
				xtext->old_ent_end = xtext->last_ent_end;
				/*xtext->io_tag = gtk_timeout_add (REFRESH_TIMEOUT,
															(GtkFunction)
															gtk_xtext_entrender_timeout,
															xtext);*/
			/*}*/
		}
	}
	...
	if (word)
	{
		if (xtext->urlcheck_function (xtext, word) > 0)
		{
				 ...
				 xtext->hilight_end != offset + len)
			{
				...
				xtext->hilight_start = offset;
				xtext->hilight_end = offset + len;
				gtk_xtext_render_ents (xtext, old_ent, word_ent,
											  xtext->adj->value, FALSE);
			}
			return FALSE;
		}
								...
								GC gc)
{
	...
	case FONT_1BYTE:
		if (dofill)
			XDrawImageString (xtext->display, xtext->draw_buf, gc, x, y, str,
									len);
		else
			XDrawString (xtext->display, xtext->draw_buf, gc, x, y, str, len);
		if (xtext->bold)
			XDrawString (xtext->display, xtext->draw_buf, gc, x + 1, y, str,
							 len);
		break;
	...
	if (xtext->underline)
		XDrawLine (xtext->display, xtext->draw_buf, gc, x, y + 1, x + str_width,
					  y + 1);
...
/*         xtext->bold = TRUE;*/
			 ...
			 (xtext->parsing_color && str[i] == ',' && xtext->nc < 3))
		{
			...
			if (str[i] == ',')
			{
				...
				if (xtext->nc)
				{
					...
					if (!mark)
						XSetForeground (xtext->display, gc,
											 xtext->palette[col_num]);
				}
			} else
			{
		...
		} else
		{
				...
				if (xtext->nc)
				{
					...
					if (xtext->parsing_backcolor)
					{
						if (col_num == 1)
							...
							xtext->backcolor = TRUE;
						if (!mark)
							XSetBackground (xtext->display, gc,
												 xtext->palette[col_num]);
			...
			case ATTR_REVERSE:
				...
				{
					XSetForeground (xtext->display, gc,
										 xtext->palette[xtext->col_fore]);
					XSetBackground (xtext->display, gc,
										 xtext->palette[xtext->col_back]);
				}
...
get_desktop_window (Window the_window)
{
	...
	for (w = the_window; w; w = parent)
	{
		if (
			 (XQueryTree
			  (GDK_DISPLAY (), w, &root, &parent, &children,
				&nchildren)) == False)
			return None;
...
create_shaded_pixmap (GtkXText * xtext, Pixmap p, int x, int y, int w, int h)
{
	...
	GdkPixbuf *pixbuf, *shaded_pixbuf;
	GdkColormap *cmap;
	GdkGC *tgc;
	guint32 color;
	...
	/* FIXME: This is probably not good for portability -- ndap */
	color = 0x00000000
	        | (xtext->tint_red << 16)
	        | (xtext->tint_green << 8)
	        | (xtext->tint_blue );

	shaded_pixbuf = gdk_pixbuf_composite_color_simple (pixbuf, w, h,
							   GDK_INTERP_NEAREST,
							   128, 32,
							   color, color);
	gdk_pixbuf_render_pixmap_and_mask (shaded_pixbuf,
								&shaded_pixmap, NULL, 128);
	gdk_pixbuf_unref (pixbuf);
	gdk_pixbuf_unref (shaded_pixbuf);
...
gtk_xtext_draw_sep (GtkXText * xtext, int y)
{
	...
	if (xtext->separator && xtext->indent)
	{
		...
		if (((GdkGCPrivate *) light)->xgc == 0
			 || ((GdkGCPrivate *) dark)->xgc == 0)
			return;
		...
		if (xtext->thinline)
		{
			if (xtext->moving_separator)
				XDrawLine (xtext->display, xtext->draw_buf,
							  ((GdkGCPrivate *) light)->xgc, x, y, x, height);
			else
				XDrawLine (xtext->display, xtext->draw_buf,
							  ((GdkGCPrivate *) dark)->xgc, x, y, x, height);
		} else
		{
			if (xtext->moving_separator)
			{
			...
			{
				XDrawLine (xtext->display, xtext->draw_buf,
							  ((GdkGCPrivate *) dark)->xgc, x - 1, y, x - 1, height);
				XDrawLine (xtext->display, xtext->draw_buf,
							  ((GdkGCPrivate *) light)->xgc, x, y, x, height);
			}
		}
...
/*   fprintf (stderr, "** USING MIT-SHM!\n");*/
	...
	xtext->img = XShmCreateImage (xtext->display,
											DefaultVisual (xtext->display,
																DefaultScreen (xtext->
																					display)),
											xtext->depth, ZPixmap, NULL, &xtext->shminfo,
											width, height);
/*   printf ("img: %p\n", xtext->img);*/
...
/*   printf ("shmid: 0x%x\n", xtext->shminfo.shmid);*/
	...
	xtext->shminfo.shmaddr = xtext->img->data =
		shmat (xtext->shminfo.shmid, 0, 0);
...
#ifdef USE_MITSHM
		xtext->tmp_pix =
			gtk_xtext_create_buffer (xtext, width + MARGIN, height);
#else
...
gtk_xtext_refresh (GtkXText * xtext, int do_trans)
{
	if (GTK_WIDGET_REALIZED (GTK_WIDGET (xtext)) &&
		 GTK_WIDGET_DRAWABLE (GTK_WIDGET (xtext)))
	{
