...
void display_players(int event, char *name);
...
void display_players(int event, char *name)
{
	...
	if (event == GGZ_PLAYER_LAG)
	{
		for (i = 0; i < num; i++)
		{
			gtk_clist_get_pixtext(GTK_CLIST(tmp), i, 2, &curname, NULL, NULL, NULL);
	...
	/* Clear current list of players */
	gtk_clist_clear(GTK_CLIST(tmp));
	gtk_clist_set_row_height(GTK_CLIST(tmp), 24);
