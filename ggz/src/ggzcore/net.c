...
{
	ggz_debug(GGZCORE_DBG_NET, "Connecting to %s:%d", net->host, net->port);
	net->fd = ggz_make_socket(GGZ_CLIENT, net->port, net->host);
...
int _ggzcore_net_send_login(struct _GGZNet *net)
{
	...
	switch (type) {
	case GGZ_LOGIN:
		ggz_debug(GGZCORE_DBG_NET, "Executing net login: GGZ_LOGIN");
		if (ggz_write_int(net->fd, REQ_LOGIN) < 0
		    || ggz_write_string(net->fd, handle) < 0
		    || ggz_write_string(net->fd, password) < 0)
			status = -1;
		break;
	case GGZ_LOGIN_GUEST:
		ggz_debug(GGZCORE_DBG_NET, "Executing net login: GGZ_LOGIN_GUEST");
		if (ggz_write_int(net->fd, REQ_LOGIN_ANON) < 0
		    || ggz_write_string(net->fd, handle) < 0)
			status = -1;
		break;
	case GGZ_LOGIN_NEW:
		ggz_debug(GGZCORE_DBG_NET, "Executing net login: GGZ_LOGIN_NEW");
		if (ggz_write_int(net->fd, REQ_LOGIN_NEW) < 0
		    || ggz_write_string(net->fd, handle) < 0)
			status = -1;
		break;
	}
...
int _ggzcore_net_send_motd(struct _GGZNet *net)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_MOTD");	
	status = ggz_write_int(net->fd, REQ_MOTD);
	if (status < 0)
...
int _ggzcore_net_send_list_types(struct _GGZNet *net, const char verbose)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_LIST_TYPES");	
	if (ggz_write_int(net->fd, REQ_LIST_TYPES) < 0
	    || ggz_write_char(net->fd, verbose) < 0)
		status = -1;
...
{	
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_LIST_ROOMS");	
	if (ggz_write_int(net->fd, REQ_LIST_ROOMS) < 0
	    || ggz_write_int(net->fd, type) < 0
	    || ggz_write_char(net->fd, verbose) < 0)
		status = -1;
...
int _ggzcore_net_send_join_room(struct _GGZNet *net, const unsigned int id)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_ROOM_JOIN");	
	if (ggz_write_int(net->fd, REQ_ROOM_JOIN) < 0
	    || ggz_write_int(net->fd, id) < 0)
		status = -1;
...
{	
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_LIST_PLAYERS");	
	status = ggz_write_int(net->fd, REQ_LIST_PLAYERS);
	if (status < 0)
...
{	
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_LIST_TABLES");	
	if (ggz_write_int(net->fd, REQ_LIST_TABLES) < 0
	    || ggz_write_int(net->fd, type) < 0
	    || ggz_write_char(net->fd, global) < 0)
		status = -1;
...
int _ggzcore_net_send_chat(struct _GGZNet *net, const GGZChatOp op, const char* player, const char* msg)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_CHAT");	
	if (ggz_write_int(net->fd, REQ_CHAT) < 0
	    || ggz_write_char(net->fd, op) < 0)
		status = -1;
	
	if (status == 0 && (op & GGZ_CHAT_M_PLAYER))
		status = ggz_write_string(net->fd, player);
	...
	if (status == 0 && (op & GGZ_CHAT_M_MESSAGE))
		status = ggz_write_string(net->fd, msg_buf);
...
int _ggzcore_net_send_table_launch(struct _GGZNet *net, const int type, char *desc, const int num_seats)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_TABLE_LAUNCH");
	if (ggz_write_int(net->fd, REQ_TABLE_LAUNCH) < 0
	    || ggz_write_int(net->fd, type) < 0
	    || ggz_write_string(net->fd, desc) < 0
	    || ggz_write_int(net->fd, num_seats) < 0)
		status = -1;
...
int _ggzcore_net_send_seat(struct _GGZNet *net, GGZSeatType seat, char *name)
{
	...
	status = ggz_write_int(net->fd, seat);
	
	if (status > 0) {
		switch (seat) {
		case GGZ_SEAT_PLAYER:
		case GGZ_SEAT_RESERVED:
			status = ggz_write_string(net->fd, name);
			break;
		case GGZ_SEAT_BOT:
			break;
		default:
...
int _ggzcore_net_send_table_join(struct _GGZNet *net, const unsigned int num)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_TABLE_JOIN");
	if (ggz_write_int(net->fd, REQ_TABLE_JOIN) < 0
	    || ggz_write_int(net->fd, num) < 0)
		status = -1;
...
int _ggzcore_net_send_table_leave(struct _GGZNet *net, int force)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_TABLE_LEAVE");
...
int _ggzcore_net_send_game_data(struct _GGZNet *net, int size, char *buffer)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_GAME: %d bytes from game", size);
	if (ggz_write_int(net->fd, REQ_GAME) < 0
	    || ggz_write_int(net->fd, size) < 0
	    || ggz_writen(net->fd, buffer, size) < 0)
		status = -1;
...
int _ggzcore_net_send_logout(struct _GGZNet *net)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Sending REQ_LOGOUT");	
	status = ggz_write_int(net->fd, REQ_LOGOUT);
...
int _ggzcore_net_read_data(struct _GGZNet *net)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Processing network events");
	...
	if (opcode < 0 || opcode >= _ggzcore_num_messages) {
		ggz_debug(GGZCORE_DBG_NET, "Bad opcode %d from server", 
			      opcode);
	...
	if (!(handler = _ggzcore_server_msgs[opcode].handler))
		ggz_debug(GGZCORE_DBG_NET, "Server msg: %s not handled yet", 
			      _ggzcore_server_msgs[opcode].name);
	else {
		ggz_debug(GGZCORE_DBG_NET, "Handling %s", 
			      _ggzcore_server_msgs[opcode].name);
		handler(net);
	}
...
static int _ggzcore_net_read_opcode(struct _GGZNet *net, GGZServerOp *op)
{
	return ggz_read_int(net->fd, (int*)op);
}
...
static int _ggzcore_net_read_server_id(struct _GGZNet *net, int *protocol, unsigned int *size)
{
	...
	if (ggz_read_string_alloc(net->fd, &msg) < 0
	    || ggz_read_int(net->fd, protocol) < 0
	    || ggz_read_int(net->fd, size) < 0)
		status = -1;

	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "%s : protocol %d: chat size %d", 
			      msg, *protocol, *size);
	else
...
static int _ggzcore_net_read_login_new(struct _GGZNet *net, char *login_status, char **password)
{
	...
	if (ggz_read_char(net->fd, login_status) < 0)
		status = -1;
	
	/* Read rest of data only if login is successful */
	if (*login_status == 0)  
		if (ggz_read_string_alloc(net->fd, password) < 0
		    || ggz_read_int(net->fd, &checksum) < 0)
			status = -1;
	
	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "RSP_LOGIN_NEW : %d, %d",
...
static int _ggzcore_net_read_login(struct _GGZNet *net, char *login_status, char *res)
{
	...
	/* Read rest of data only if login is successful */
	if (*login_status == 0)
		if (ggz_read_int(net->fd, &checksum) < 0
		    || ggz_read_char(net->fd, res) < 0)
			status = -1;

	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "RSP_LOGIN : %d, %d, %d", 
...
static int _ggzcore_net_read_login_anon(struct _GGZNet *net, char *login_status)
{
	...
	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "RSP_LOGIN_ANON : %d, %d", 
			      *login_status, checksum);
	else
...
static int _ggzcore_net_read_motd(struct _GGZNet *net, int *lines, char ***buffer)
{
	...
	if (ggz_read_int(net->fd, lines) < 0)
		status = -1;
	...
	if (status == 0) {
		...
		for (i = 0; i < *lines; i++)
			if (ggz_read_string_alloc(net->fd, &motd[i]) < 0) {
				status = -1;
				break;
			}
	}
	...
	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "MSG_MOTD : %d lines", *lines);
	else
...
static int _ggzcore_net_read_logout(struct _GGZNet *net, char *logout_status)
{
	...
	if (ggz_read_char(net->fd, logout_status) < 0)
		status = -1;

	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "RSP_LOGOUT : %d", *logout_status);
	else
...
static int _ggzcore_net_read_num_rooms(struct _GGZNet *net, int *num)
{
	return ggz_read_int(net->fd, num);
}
			   ...
			   char **desc)
{
	...
	if (ggz_read_int(net->fd, id) < 0
	    || ggz_read_string_alloc(net->fd, name) < 0
	    || ggz_read_int(net->fd, game) < 0)
		return  -1;
	
	if (net->room_verbose && ggz_read_string_alloc(net->fd, desc) < 0)
		return -1;
	
	ggz_debug(GGZCORE_DBG_NET, "Room: %d (%s) plays %d", *id, *name, 
		      *game);
...
static int _ggzcore_net_read_type(struct _GGZNet *net, struct _GGZGameType *type)
{
	...
	if (ggz_read_int(net->fd, &id) < 0
	    || ggz_read_string_alloc(net->fd, &name) < 0
	    || ggz_read_string_alloc(net->fd, &version) < 0
	    || ggz_read_string_alloc(net->fd, &engine) < 0
	    || ggz_read_string_alloc(net->fd, &p_version) < 0
	    || ggz_read_char(net->fd, &players) < 0
	    || ggz_read_char(net->fd, &bots) < 0
	    || ggz_read_string_alloc(net->fd, &desc) < 0
	    || ggz_read_string_alloc(net->fd, &author) < 0
	    || ggz_read_string_alloc(net->fd, &url) < 0)
		return -1;
	    
	ggz_debug(GGZCORE_DBG_NET, "Read info for game %d: %s", id, name);
...
static int _ggzcore_net_read_room_join(struct _GGZNet *net, char *status)
{
	if (ggz_read_char(net->fd, status) < 0)
		return -1;

	ggz_debug(GGZCORE_DBG_NET, "RSP_ROOM_JOIN from server : %d", *status);
...
static int _ggzcore_net_read_player(struct _GGZNet *net, char **name, int *table)
{
	if (ggz_read_string_alloc(net->fd, name) < 0
	    || ggz_read_int(net->fd, table) < 0)
		return -1;

	ggz_debug(GGZCORE_DBG_NET, "Player %s at table %d", *name, *table);
...
{
	if (ggz_read_int(net->fd, id) < 0
	    || ggz_read_int(net->fd, room) < 0
	    || ggz_read_int(net->fd, type) < 0
	    || ggz_read_string_alloc(net->fd, desc) < 0
	    || ggz_read_char(net->fd, state) < 0
	    || ggz_read_int(net->fd, seats) < 0)
		return -1;
	
	ggz_debug(GGZCORE_DBG_NET, "Read info for table %d", *id);
...
static int _ggzcore_net_read_seat(struct _GGZNet *net, GGZSeatType *seat, char **name)
{
	...
	*seat = type;
	...
	case GGZ_SEAT_PLAYER:
	case GGZ_SEAT_RESERVED:
		ggz_read_string_alloc(net->fd, name);
		break;
	case GGZ_SEAT_BOT:
		break;
	default:
...
static int _ggzcore_net_read_rsp_chat(struct _GGZNet *net, char *status)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "RSP_CHAT from server : %d",
		      *status);
			   ...
			   char **msg)
{
	...
	if (ggz_read_char(net->fd, &opcode) < 0
	    || ggz_read_string_alloc(net->fd, name) < 0)
		status = -1;
	...
	*op = opcode;
	...
	if (status == 0)
		ggz_debug(GGZCORE_DBG_NET, "Chat opcode = %d", opcode);	
	else
				     ...
				     char **name)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "Update opcode = %d", opcode);
	*op = opcode;
...
{
	if (ggz_read_int(net->fd, id) < 0
	    || ggz_read_int(net->fd, seat) < 0
	    || ggz_read_string_alloc(net->fd, player) < 0)
		return -1;
...
static int _ggzcore_net_read_table_launch(struct _GGZNet *net, char *status)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "RSP_TABLE_LAUNCH from server : %d", *status);
...
static int _ggzcore_net_read_table_join(struct _GGZNet *net, char *status)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "RSP_TABLE_JOIN from server : %d", *status);
...
static int _ggzcore_net_read_table_leave(struct _GGZNet *net, char *status)
{
	...
	ggz_debug(GGZCORE_DBG_NET, "RSP_TABLE_LEAVE from server : %d", *status);
...
static int _ggzcore_net_read_game_data(struct _GGZNet *net, int *size, char *buffer)
{
	if (ggz_read_int(net->fd, size) < 0
	    || ggz_readn(net->fd, buffer, *size) < 0)
		return -1;

	ggz_debug(GGZCORE_DBG_NET, "RSP_GAME: %d bytes from server", *size);
 ...
 void _ggzcore_net_handle_list_rooms(struct _GGZNet *net)
{
	...
	if (num < 0) {
		ggz_debug(GGZCORE_DBG_NET, "Error loading rooms");
