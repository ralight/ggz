...
public class Mod implements ModGGZ, ModGame {
    ...
    private ArrayList<SpectatorSeat> spectator_seats;
    ...
    public SpectatorSeat get_spectator_seat(int num) {
        ...
        if (num >= 0 && num < this.num_spectator_seats) {
            ...
            if (entry > -1)
                seat = this.spectator_seats.get(entry);
        }
    ...
    private void send_game_launch() throws IOException {
        ...
        if (this.seats != null) {
            for (Iterator<Seat> iter = this.seats.iterator(); iter.hasNext();) {
                Seat seat = iter.next();
                ehandler.handle_seat(seat);
            }
        }
        ...
        if (this.spectator_seats != null) {
            for (Iterator<SpectatorSeat> iter = this.spectator_seats.iterator(); iter
                    .hasNext();) {
                SpectatorSeat seat = iter.next();
                ehandler.handle_spectator_seat(seat);
            }
        }
