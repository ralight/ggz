...
echo -n "[automake]"
set -o pipefail 2>/dev/null && { ((cd $srcdir && automake --foreign --add-missing 2>&1) | (grep -v installing || true)) || { echo "automake failed." ; exit; } } || { (cd $srcdir && automake --foreign --add-missing) || { echo "automake failed." ; exit; } }
