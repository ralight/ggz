...
int check_table(int p_index, TableInfo table ) {
  ...
  dbg_msg ("Player %d launching table of type %d", p_index, table.type_index);
  ...
  for (i=0; i<table.num_seats; i++) 
    dbg_msg ("Players[i]: %d, fd of %d", table.players[i], table.player_fd[i]);
...
static int pass_options(int t_index) {
  ...
  if (FAIL(es_write_int(fd, REQ_GAME_LAUNCH)) ||
      ...
      FAIL(es_write_char(fd, game_tables.info[t_index].comp_players)))
    return(-1);
  ...
  for (i = 0; i < game_tables.info[t_index].num_humans; i++) {
	...
	FAIL(es_write_int(fd, game_tables.info[t_index].player_fd[i])))
      return(-1);
  }
  
  /* Make sure game server says everything is OK */
  if (FAIL(es_read_int(fd, &op)) ||
      FAIL(es_read_char(fd, &status)))
    return(-1);
...
static int handle_game(int request, int index, int fd) {
  ...
  default:
    dbg_msg("Table %d requested unimplemented op %d", index, op);
    status = -1;
  }
