...
static list_t * file_parser(const char *path);
		        ...
		        const char *key, const char *def)
{
	...
	s_entry = list_search(f_data->section_list, (void*)section);
	if(s_entry == NULL)
		goto do_default;
	s_data = list_get_data(s_entry);

	/* Locate the requested Key */
	e_srch.key = (char*)key;
	e_entry = list_search(s_data->entry_list, &e_srch);
	if(e_entry == NULL)
		goto do_default;
	e_data = list_get_data(e_entry);
		...
		*argvp = (char **) calloc((*argcp + 1), sizeof(char *));
		...
		do {
			...
			if((tmp2 = malloc(p-tmp+1)) == NULL)
				err_sys_exit("malloc failed in conf_read_list");
		      ...
		      const char *key, const char *value)
{
	...
	if(s_entry == NULL) {
		...
		if(list_insert(f_data->section_list, (void*)section) < 0) {
			dbg_msg(GGZ_DBG_CONFIGURATION,
	...
	if(list_insert(s_data->entry_list, &e_data) < 0) {
		dbg_msg(GGZ_DBG_CONFIGURATION,
			      "ggzcore_confio_write_string: insertion error");
		return -1;
	}
...
int conf_remove_section(int handle, const char *section)
{
	...
	list_delete_entry(f_data->section_list, s_entry);
...
int conf_remove_key(int handle, const char *section, const char *key)
{
	...
	e_data.key = (char*)key;
	e_entry = list_search(s_data->entry_list, &e_data);
	if(e_entry == NULL)
		return 1;

	/* Remove the key's list entry */
	list_delete_entry(s_data->entry_list, e_entry);
...
int conf_commit(int handle)
{
	...
	if((c_file = fopen(f_data->path, "w")) == NULL) {
		dbg_msg(GGZ_DBG_CONFIGURATION, "Unable to write config file %s",
			      f_data->path);
		return -1;
	}

	/* Now step through each section, writing each entry from it's list */
	s_entry = list_head(f_data->section_list);
	while(s_entry) {
		s_data = list_get_data(s_entry);
		...
		e_entry = list_head(s_data->entry_list);
		while(e_entry) {
			e_data = list_get_data(e_entry);
			...
			e_entry = list_next(e_entry);
		}

		s_entry = list_next(s_entry);
	}
...
{
	list_entry_t	*f_entry;
	conf_file_t	*f_data;

	f_entry = list_head(file_list);
	while(f_entry) {
		f_data = list_get_data(f_entry);
		list_destroy(f_data->section_list);
		free(f_data->path);
		free(f_data);
		f_entry = list_next(f_entry);
	}
...
int	conf_parse(const char *path, const unsigned char options)
{
	...
	if(!file_list)
		file_list = list_create(NULL, NULL, NULL, LIST_ALLOW_DUPS);
	...
	if(access(path, F_OK)) {
		...
		if(opt_create) {
			...
			if(t_file != -1)
				...
				err_sys("Unable to create file %s", path);
				return -1;
			}
		}
	}
	...
	if(opt_rdonly && access(path, R_OK)) {
		err_sys("Unable to read file %s", path);
		return -1;
	}
	if(opt_rdwr && access(path, R_OK | W_OK)) {
		err_sys("Unable to read or write file %s", path);
		return -1;
	}
	...
	if(list_insert(file_list, file_data) < 0) {
		/* ACK - PUKE */
		list_destroy(section_list);
		return -1;
	}
...
static list_t * file_parser(const char *path)
{
	...
	s_list = list_create(section_compare,
			      section_create,
			      section_destroy,
			      LIST_REPLACE_DUPS);
	if(!s_list)
		...
		if(varvalue == NULL) {
			...
			if(varname[0] == '['
			   && varname[strlen(varname)-1] == ']') {
				...
				varname++;
				if(list_insert(s_list, varname) < 0)
					err_sys_exit("list insert error: file_parser");
				s_entry = list_search(s_list, varname);
				s_data = list_get_data(s_entry);
			} else
				err_msg("Syntax error, %s (line %d)",
					 path, linenum);
			continue;
		}
...
static void *section_create(void *data)
{
	...
	dst->entry_list = list_create(entry_compare,
				       entry_create,
				       entry_destroy,
				       LIST_REPLACE_DUPS);
	if(!dst->entry_list) {
...
static void section_destroy(void *data)
{
	...
	s_data = data;
	free(s_data->name);
	list_destroy(s_data->entry_list);
	free(s_data);
}
