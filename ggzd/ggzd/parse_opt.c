...
static const struct poptOption args[] = {
  ...
  {"log",  'l', POPT_ARG_INT, &opt.log_level,  0, "Verbosity of logging", "LEVEL"},
  {"port", 'p', POPT_ARG_INT, &opt.main_port,  0, "NetGames main port", "PORT"},
...
void parse_args( int argc, char* argv[]) {
  ...
  while( (rc = poptGetNextOpt(context)) != -1 ) {
    ...
    case POPT_ERROR_OVERFLOW:
      err_msg("%s: %s", poptBadOption(context, 0), poptStrerror(rc));
      poptFreeContext(context);
      exit(-1);
      break;
    }
