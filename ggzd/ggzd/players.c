...
static void io_handler(int p_index) {
  ...
  for(;;) {
    ...
    if (FAIL(status = select(fd_max, &read_fd_set, NULL, NULL, NULL))) {
      if (errno == EINTR)
	continue;
      else
    ...
    if (FD_ISSET(p_fd, &read_fd_set)) {
      ...
      /* FIXME: Temporary hack until player implements MSG_GAME! */
      if (playing) {
	...
	fd = p_fd; 
	if (FAIL(status = handle_player(op, p_index, &fd)))
	  break;
	
	dbg_msg("Player %d op handled successfully", p_index);
	
	if (status == 1) { /* Player launched or joined a game */
    ...
    if (playing && FD_ISSET(t_fd, &read_fd_set)) {
      ...
      if (status <= 0) { /* Game over */
	dbg_msg("Player %d's game is over (msg from game)", p_index);
	playing = 0;
	close(t_fd);
	FD_CLR(t_fd, &active_fd_set);
	t_fd = -1;
...
int handle_player(int request, int p_index, int* fd) {
  ...
  case REQ_USER_STAT:
  default:
    dbg_msg("Player %d (uid: %d) requested unimplemented op %d", p_index,
	    players.info[p_index].uid, op);
    status = -1;
  }
...
static void remove_player(int p_index) {
  ...
  dbg_msg("Removing player %d (uid: %d)", p_index, players.info[p_index].uid);
  pthread_rwlock_wrlock(&players.lock);
...
static int new_login( int p ) {
  ...
  dbg_msg("Successful new login of %s: UID = %d", players.info[p].name,
	  players.info[p].uid );
...
static int read_table_info(int p_index, int* fd) {
  ...
  if (status == 0 ) {
    table.num_humans = table.num_seats - num_comp_play(table.comp_players);
  ...
  case E_BAD_OPTIONS:
    free(options);
    dbg_msg("Player %d's table launch failed with err %d", p_index, status);
    break;
...
static int join_table(int p_index, int* fd) {
  ...
  if (game_tables.info[t_index].open_seats == 0) {
	...
	game_tables.info[t_index].players[i] = p_index;
	game_tables.info[t_index].player_fd[i] = fds[0];
	dbg_msg("Player %d sitting in seat %d", p_index, i);
	break;
      }
  ...
  case E_SERVER_FULL:
  case E_BAD_OPTIONS:
    dbg_msg("Player %d's table join failed with err %d", p_index, status);
    break;
  case 0: /* Everything OK */
    pthread_rwlock_wrlock(&players.lock);
    players.info[p_index].table_index = t_index;
...
static int table_list(int p_index) {
  ...
  for (i = 0; i < count; i++) {
	...
	FAIL(es_write_int(fd, info[i].num_humans - info[i].open_seats)))
      return(-1);
    for (j = 0; j < info[i].num_humans - info[i].open_seats; j++ ) {
      tmp = info[i].players[j];
      pthread_rwlock_rdlock(&players.lock);
      strncpy(name, players.info[tmp].name, MAX_USER_NAME_LEN+1);
      pthread_rwlock_unlock(&players.lock);
