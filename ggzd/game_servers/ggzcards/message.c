 ...
 * $Id: message.c 4000 2002-04-16 19:14:01Z jdorje $
 *
...
{
	(void) net_send_global_text_message(p, mark, get_global_message(mark));
}
...
void send_all_global_messages(player_t p)
{
	...
	for (gml = game.message_head; gml != NULL; gml = gml->next)
		(void) net_send_global_text_message(p, gml->mark, gml->message);
}
