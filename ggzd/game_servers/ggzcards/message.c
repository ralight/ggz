 ...
 * $Id: message.c 2197 2001-08-23 09:34:23Z jdorje $
 *
...
void set_player_message(player_t p)
{
	if (game.which_game == GGZ_GAME_UNKNOWN)
		...
		return;
	ggzdmod_debug("Setting player %d/%s's message.", p,
		      ggz_seats[p].name);
...
static void dosend_global_message(char *mark, char *message, player_t p)
{
	...
	ggzdmod_debug("Sending global message '%s' to player %d/%s: %s", mark,
		      p, ggz_seats[p].name, message);
