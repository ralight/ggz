 ...
 * $Id: message.c 2229 2001-08-25 14:52:34Z jdorje $
 *
...
void send_player_message(seat_t s, player_t p)
{
	...
	ggzd_debug("Sending seat %d/%s's message to player %d/%s: %s", s,
		      game.seats[s].ggz->name, p, ggzd_seats[p].name,
...
static void dosend_global_message(char *mark, char *message, player_t p)
{
	...
	ggzd_debug("Sending global message '%s' to player %d/%s: %s", mark,
		      p, ggzd_seats[p].name, message);
	if (es_write_int(fd, WH_MESSAGE_GLOBAL) < 0
	    || es_write_string(fd, mark) < 0
	    || es_write_string(fd, message) < 0)
		ggzd_debug("ERROR: " "dosend_global_message: es error.");
...
void send_last_hand()
{
	...
	for (p = 0; p < game.num_players; p++)
		if (strlen(ggzd_seats[p].name) > width)
			width = strlen(ggzd_seats[p].name);
...
static void send_cumulative_scores()
{
	...
	for (p = 0; p < game.num_players; p++) {
		widths[p] = strlen(ggzd_seats[p].name);
		if (widths[p] < 4)
			widths[p] = 4;
		buf_len +=
			snprintf(buf + buf_len, sizeof(buf) - buf_len,
				 "%*s%s", widths[p], ggzd_seats[p].name,
