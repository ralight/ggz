 ...
 * $Id: message.c 2405 2001-09-08 23:03:15Z jdorje $
 *
...
/* NOTE ON "MESSAGES" - Each seat has a message.  The client should display
   ...
   It may contain information such as contract, trump, etc.  - There are
...
static void dosend_global_message(char *mark, const char *message, player_t p)
{
	int fd = ggzd_get_player_socket(p);
	if (ggzd_get_seat_status(p) == GGZ_SEAT_BOT
	...
	ggzd_debug("Sending global message '%s' to player %d/%s: %s", mark,
		   p, ggzd_get_player_name(p), message);
	if (fd < 0
