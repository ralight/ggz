 ...
 * $Id: spades.c 2340 2001-09-02 07:50:46Z jdorje $
 *
...
static int spades_get_bid()
{
	...
	char pard =
		(game.bid_count >=
		 2) ? game.players[(game.next_bid + 2) % 4].bid.sbid.val : 0;
	...
	if (GSPADES.nil_value > 0
	    && (game.bid_count < 2 || pard >= GSPADES.minimum_team_bid))
	...
	for (p = 0; p < 4; p++) {
		if (game.players[p].bid.sbid.spec == SPADES_NIL) {
			...
			ggzd_debug("Player %d/%s earned %d for going nil.",
				   (int) p, ggzd_get_player_name(p), score);
			game.players[p].score += score;
