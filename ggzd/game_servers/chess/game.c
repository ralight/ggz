    ...
    case CHESS_EVENT_LAUNCH:
      ...
      if (ggzdmod_count_seats(game_info.ggz, GGZ_SEAT_BOT) == 1) {
		...
		if (ai_type == AI_GGZCHESS) {
			...
			ret = gnuchess_launch();
			if(!ret) {
				ggzdmod_log(game_info.ggz, "error: gnuchess launch failed!");
				return -1;
			}
		}
      }
    ...
    case CHESS_EVENT_START:
      ...
      && (game_info.turn % 2 == seatnum)) {
		if (ai_type == AI_GGZCHESS) {
			...
			if (!gnuchess_check()) {
				ggzdmod_log(game_info.ggz, "error: gnuchess has died");
				return -1;
			}
		}
        botmove[0] = from % 8 + 'A';
...
static void game_save(char *fmt, ...)
{
  ...
  if(!savegame) {
    savegamepath = ggz_strdup(GGZDSTATEDIR "/gamedata/Chess/savegame.pgn.XXXXXX");
