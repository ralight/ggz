...
/* IO: From this point on, the code is mine
 * (so, don't complain about it to Brent, complain to me!) */

/* Define the handlers */
typedef void (*GGZHandler)(int id, void *handler_data);

/* Event types */
#define GGZ_EVENT_LAUNCH  0
#define GGZ_EVENT_JOIN    1
#define GGZ_EVENT_LEAVE   2
#define GGZ_EVENT_QUIT    3
#define GGZ_EVENT_PLAYER  4

/* Set the handler for the specific event */
void ggz_set_handler(unsigned int event, const GGZHandler handler);

/* Open the ggz socket, and wait for events, calling the handlers when it is 
 * the case */
int ggz_main();
